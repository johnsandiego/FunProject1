...
int main(int argc, char *argv[])
{
 //Variables used by OS
 	...
 	struct PCB *RQ, *tmp, *RQT, *Current, *Sem ;
...
int	ExecuteProc(struct PCB *CurrentProc)
    ...
    while(1)
    {
      ...
      switch (opcode) {
        ...
        case 36: OP36(IR); PC++; break;
        case 37: OP37(IR, a ,b); PC++; break;
...
// if counter is less th -1
/* put the current process to sem queue struct


*/
void wait(struct semaphore *sem, struct PCB *current) {
	sem->count -= 1;
	if(sem->count < 0);
    sem->Sem_Queue = current;
    while(sem->count < 0);
		}
/*
counter is incremented when called
if counter <= 0
something is in the sem queue take it out and put in the ready queue
*/
void signal(struct semaphore *sem, struct PCB *current){
		sem->counter += 1;
    if(sem->count >= 0){
      *current = sem->Sem_Queue;
